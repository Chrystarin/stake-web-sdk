<script lang="ts">
	...
	import { TOP_SLOT_MULTS, WHEEL_LAYOUT } from '../game/constants';
	import { playSound } from '../game/sound';
	import { stateGame } from '../game/stateGame.svelte';
	import { staticUrl } from '../lib/staticUrl';

	/**
	 * The 4-page quick guide, ported from One-Eyed Willy's Plinko (apps/plinko QuickGuideModal.svelte):
	 * the same wooden frame, paging, keys and stylesheet. It opens once after the intro splash and again
	 * from Menu → How to Play?.
	 *
	 * Two things the Plinko's has and this does not, because the art was never made for this game:
	 *   • the looping gameplay clip per page. Every page shows ONE placeholder in the well instead, a
	 *     still of the table (`quick_guide_placeholder*.webp`, a 16:9 cut and a 586x960 one, the shapes
	 *     of the Plinko's two sets of clips). A page that is given a clip later only has to put a
	 *     `<video>` in the well over it.
	 *   • the title logo over the panel. The game's name is a placeholder (see README), so there is no
	 *     logo to hang there, and the height budgets in the stylesheet are the panel's alone.
	 *
	 * ⚠️ HOUSE STYLE for the copy: no em dashes, no semicolons.
	 */
	type QuickGuidePage = {
		title: string;
		/** The body copy, ONE ENTRY PER LINE ON SCREEN (see `.qg-content-line`). */
		content: readonly string[];
		/**
		 * Italic caveat under the copy. Omitted on the pages that carry none.
		 * ⚠️ ONE LINE in the mobile frame (about 48 characters): the counter sits directly under it, and
		 * the binding page there is five rows of copy plus a one-row note. A second row lands on the counter.
		 */
		note?: string;
	};

	const topSlotMax = Math.max(...TOP_SLOT_MULTS);
	const roomMax = Math.max(...WHEEL_LAYOUT).toLocaleString('en-US');

	const PAGES: readonly QuickGuidePage[] = [
		{
			title: 'BET & SPIN',
			content: [
				'Pick your chip. Tap the spots you want to cover. Hit SPIN!',
				'If the wheel stops on a spot you covered, that chip wins.',
			],
			note: 'Total Bet = your chip × the spots you cover',
		},
		{
			title: 'TOP SLOT MULTIPLIER',
			content: [
				`Before every spin the Top Slot picks one spot and a multiplier of up to ${topSlotMax}x.`,
				'Stop the wheel on that same spot to multiply your win!',
			],
			note: 'It counts only on the spot the Top Slot picked',
		},
		{
			title: 'BONUS ROOMS',
			content: [
				'Stop on a bonus room you covered and it opens for you to play.',
				`Four rooms to play, paying up to ${roomMax}x your chip!`,
			],
			note: 'A room you did not cover pays nothing',
		},
		{
			title: 'BUY BONUS',
			content: [
				'Tap Buy Bonus to skip the wheel and open a bonus room straight away.',
				'Choose your room, or let Random Bonus pick one for you!',
			],
		},
	];
	...
	/**
	 * How long to hold after the splash reports itself finished before the guide comes in.
	 * `introLoaderComplete` flips at the START of the splash's fade-out, so opening on the raw flag
	 * paints this backdrop over a splash that is still on screen. Mirrors `FADE_OUT_MS` in
	 * LoaderCasinoTvLogo.svelte: keep the two in step.
	 */
	const SPLASH_HANDOVER_MS = 400;

	/** The guide opens ONCE per session, after the intro splash clears. */
	let autoOpened = false;
	$effect(() => {
		if (!stateGame.introLoaderComplete || autoOpened || isReplay || isHarness) return;
		autoOpened = true;
	...
	/** Every open starts at page 1: reopening from the menu should not resume mid-walkthrough. */
	$effect(() => {
	...
	/** "Click anywhere outside the panel to close", decided by hit-testing the panel. */
	...
	/**
	 * Arrow keys page the guide and Escape closes it. ArrowRight follows the right-hand button: on the
	 * last page that button is DONE, so the key closes the guide too.
	 */
...
{#if stateGame.quickGuideOpen}
	<!-- The backdrop IS the dismiss target: a click anywhere off the panel closes the guide. -->
	<div class="qg-backdrop" role="presentation" onclick={onBackdropClick}>
		<!-- BOTH frames are handed over as custom properties and the stylesheet picks between them:
		     orientation is a media query. The URLs come from JS because `staticUrl` is what resolves
		     them to the preloaded `blob:` handles. -->
		<div
			class="qg-modal"
			role="dialog"
			aria-modal="true"
		...
		>
			...
			<div class="qg-inner">
				<!-- One placeholder for every page until the game has clips of its own. Both cuts are handed
				     over and the stylesheet picks by orientation, as it does for the frames. -->
				<div
					class="qg-video"
					aria-hidden="true"
					style:--qg-still-wide="url({staticUrl('img/quick_guide/quick_guide_placeholder.webp')})"
					style:--qg-still-mobile="url({staticUrl(
				...
				<!-- One <span> a line, not one <p> a line: the lines are one paragraph that carries its
				     own breaks, so they share a line-height and nothing sits between them. -->
				<p class="qg-content">
...
<style>
	.qg-backdrop {
		 ...
		 */
		--qg-panel-w: calc(
			min(94vw, calc(1000 * var(--ui-px)), calc(78vh * 1311 / 735)) * var(--qg-panel-scale)
		);
	...
	/* The well: the Plinko's looping-video box, holding the placeholder still until there are clips. */
	.qg-video {
		position: relative;
		width: calc(var(--qg-video-width) * var(--qg-video-scale));
		transform: translate(calc(var(--qg-video-x) * 1cqw), calc(var(--qg-video-y) * 1cqw));
		/* The CLIPS' own aspect, per orientation — 16:9 landscape, 586x960 in the mobile frame. A knob rather than a constant because the two cuts are
		...
		flex: 0 0 auto;
		overflow: hidden;
		/* The placeholder still. `cover` is a no-op: each cut is its well's own aspect. */
		background: #000000 var(--qg-still-wide) center / cover no-repeat;
	...
	@media (max-aspect-ratio: 1/1) {
		.qg-backdrop {
			 ...
			 */
			--qg-panel-w: calc(min(88.3vw, calc(1000 * var(--ui-px)), 54vh) * var(--qg-panel-scale));
		...
		.qg-video {
			background-image: var(--qg-still-mobile);
		}
	}
</style>
