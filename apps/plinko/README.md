...
The **wallet only changes through RGS** (`/wallet/play`, `/bet/action`, `/wallet/end-round`). The client cannot safely credit wins in the browser.

- Session **spin meter carry-over** is sent on each bet via `buildBetMetaPlayConditions()` (`spin_meter_start`, `bonus_meter_start`, `row_count`, …). Math selects books where `plinkoDrop.spinMeterStart` matches and includes `freeSpinTrigger` + payout when the meter fills.
- If the served book omits `freeSpinTrigger` but the session meter is full, `ensureFreeSpinWhenSessionMeterFull()` may call RGS `/bet/action` or use a deterministic fallback segment (`features/freeSpin/`).
- Pending free-spin credits are flushed in `playBet` **before** end-round (`flushPendingFreeSpinWalletBeforeEndRound`).
