...
**Free spin (spin meter).** A ball landing in the **center 0× pocket** fills the spin meter (`onSpinSlotLand`). Base bets only *fill* the meter (they carry it, but never fire the feature in-drop — keeps base RTP at the flat per-ball board EV on every tier); when the meter is full the `freeSpinTrigger` is emitted by the auto-fired trigger-mode book (see *Meter persistence & triggering* below) and the client opens the wheel (`runFreeSpinTriggerFlow`). An `NX` segment multiplies that round's drop win; a `BONUS` segment chains into a bonus round. Wallet credit is the book `finalWin` / `payoutMultiplier`, settled by RGS `/wallet/end-round`.

**Bonus (bonus meter).** A ball that contacts a **bonus (coin) peg** (`hitBonusPeg`, `onCoinPegHit`) fills the bonus meter. As with the spin meter, base bets only fill it; when full, the auto-fired trigger-mode book emits `bonusRoulette` (entry free balls) then one `bonusRound` per level. Free balls drop one at a time (`playOneBonusBall` → `bonusBallDrop`), each landing on its **book-authored** slot (`takeAuthoritativeBonusOutcome`), accumulating into `bonusSessionWinAmount`.
