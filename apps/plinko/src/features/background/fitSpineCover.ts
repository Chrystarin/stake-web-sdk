...
export function fitSpineCover(
	...
	const bw = bounds.width > 0 ? bounds.width : spine.skeleton.data.width;
	const bh = bounds.height > 0 ? bounds.height : spine.skeleton.data.height;
	const bx = bounds.width > 0 ? bounds.x : spine.skeleton.data.x;
	const by = bounds.height > 0 ? bounds.y : spine.skeleton.data.y;
